...
const UserList = ({ setTeam, team }) => {
  const { data: teamList, isLoading } = useGetTeamListQuery();
  const [selectedUsers, setSelectedUsers] = useState([]);

  useEffect(() => {
    if (!isLoading && teamList) {
      if (team?.length > 0) {
        const initialSelectedUsers = teamList.filter(user => team.includes(user?._id));
        setSelectedUsers(initialSelectedUsers);
      } else {
        setSelectedUsers([]); // Clear selected users if no team is provided
      }
    }
  }, [team, teamList, isLoading]);

  useEffect(() => {
    if (team && team.length > 0 && !isLoading) {
      setSelectedUsers(teamList.filter(user => team.includes(user?._id)));
    }
  }, [team, teamList, isLoading]);
  ...
  return (
    <div>
      <p className="text-gray-700">Assign Project To:</p>
      <Listbox value={selectedUsers} onChange={handleChange} multiple>
        <div className="relative mt-1">
          ...
          <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
            <Listbox.Options className="z-50 absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 active:outline-none sm:text-sm">
